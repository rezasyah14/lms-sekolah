    ...
    <aside className="w-64 shrink-0 h-screen sticky top-0 flex flex-col border-r border-white/5 bg-slate-950/50 backdrop-blur-xl">
      <div className="p-6 border-b border-white/5">
        <Link href="/" className="flex items-center gap-3">
          <div className="w-9 h-9 gradient-brand rounded-xl flex items-center justify-center text-white font-bold text-lg shadow-lg shadow-indigo-500/30">E</div>
          <div>
      ...
      <div className="p-4 border-t border-white/5">
        <div className="glass-card p-3 flex items-center gap-3">
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="text-xs text-slate-500 truncate">{userClass ?? (role === "teacher" ? "Guru" : "Siswa")}</div>
          </div>
          <Link href="/login" title="Keluar" className="text-slate-500 hover:text-slate-300 transition-colors text-sm">↩</Link>
        </div>
      </div>
    </aside>
  );
